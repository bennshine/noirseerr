...
const Studio = ({route, navigation}) => {
    ...
    useEffect(() => {
        const fetchStudios = async () => {
            try {
                const response = await fetch(`${overserrUrl}/api/v1/discover/movies/studio/${id}?page=${page}`,  {
                    headers: {
                ...
                {item.posterPath ? (
                    ...
                    >
                        <LinearGradient
                            ...
                            style={styles.linearGradientStudio}
                        />
                    </ImageBackground>
                )}
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                <Image
                    source={{
                        uri: movieData && movieData.studio && movieData.studio.logoPath
                            ? `https://request.negro.media/imageproxy/t/p/w780_filter(duotone,ffffff,bababa)${movieData.studio.logoPath}`
            ...
            <FlatList
                data={movieData && movieData.results ? movieData.results : []} // Add a check for tvData before accessing tvData.results
